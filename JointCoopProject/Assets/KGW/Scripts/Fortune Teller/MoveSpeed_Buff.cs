...
{
    public string _buffName => $"이동속도 {(_buffCategory == BuffCategory.Blessing ? "증가 버프" : "감소 버프")}";
    public string _buffDescription => $"이동속도를 {(_buffCategory == BuffCategory.Blessing ? $"{_buffOffset}증가" : $"{_buffOffset}감소")}";
    ...
    public void BuffReceive(PlayerStatManager playerStatus)
    {
        ...
        {
            playerStatus._moveSpeed -= _buffOffset; // 저주시 마이너스 스텟 적용
        }
