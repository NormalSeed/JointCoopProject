...
public class EliteOrc_Attack1 : EliteOrcState
{
    ...
    public override void FixedUpdate()
    {
        if (_rushDelay <= 0f)
        {
            _controller.gameObject.transform.position = Vector2.MoveTowards(_controller.gameObject.transform.position, _controller._rushDestination, _controller._model._moveSpd * 2 * Time.deltaTime);
        }
    }
